...
const HKFirebox = () => {
    ...
    const [logs, setLogs] = useState(["HK Firebox Modu Hazır."]);
    const [selectedFile, setSelectedFile] = useState(null);
    const [isSelectingFile, setIsSelectingFile] = useState(false);
    const [reportPath, setReportPath] = useState(null);

    const activeMode = 'firebox';
    const config = {
        title: "HK Firebox Veri",
        desc: "Firebox durumu normal olmayan araçları listeler.",
        icon: HardDrive, // TODO: Change Icon?
        color: "text-red-400",
        bg: "bg-purple-500/10",
    ...
    const checkExistingReport = async () => {
        if (window.electronAPI && window.electronAPI.checkReportExists) {
            const filename = "Hıkvısıon Firebox Veri (2.Yöntem).xlsx";
