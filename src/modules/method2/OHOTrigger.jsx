    ...
    const handleSelectFiles = async () => {
        ...
        try {
            if (window.electronAPI && window.electronAPI.selectFile) {
                // In this project it's select-file (returns single) but in script it expects a list.
                // Let's use a logic to allow multiple selection if UI allows it.
                // Looking at Sidebar.jsx, there is no multi-select tool.
                // I'll stick to selectFile for now or see if there is another tool.
                const path = await window.electronAPI.selectFile();
                if (path) {
                    setSelectedFiles(prev => [...prev, path]);
                    setLogs(prev => [...prev, `Dosya eklendi: ${path.split(/[\\/]/).pop()}`]);
                }
            }
        } finally {
            ...
            <div className={`flex justify-between items-center p-6 rounded-2xl border transition-colors duration-300 ${config.bg} ${config.border}`}>
                ...
                <div className="flex items-center gap-3 no-drag">
                    ...
                    >
                        ...
                        <span>{selectedFiles.length > 0 ? `${selectedFiles.length} Dosya Seçildi` : 'Tetik Veri Seç'}</span>
                    </button>
                    {status === 'running' ? (
