...
import { Users, FileSpreadsheet, X, Play, CheckCircle2, AlertTriangle } from 'lucide-react';
import Terminal from '../../ui/Terminal';

const HKPassenger = () => {
    const [status, setStatus] = useState('idle');
    const [progress, setProgress] = useState(0);
    const [logs, setLogs] = useState(["HK Yolcu Modu Hazır."]);
    const [selectedFile, setSelectedFile] = useState(null);
    const [isSelectingFile, setIsSelectingFile] = useState(false);
    const [reportPath, setReportPath] = useState(null);

    const activeMode = 'yolcu';
    const config = {
        title: "HK Yolcu Yükü Faktörü",
        desc: "Yolcu Yükü Faktörü durumunu analiz eder.",
        icon: Users,
    ...
    const checkExistingReport = async () => {
        if (window.electronAPI && window.electronAPI.checkReportExists) {
            const filename = "Hıkvısıon Yolcu Yükü Veri (2.Yöntem).xlsx";
    ...
    const refreshSettingsLog = async (isUpdate = false) => {
        if (window.electronAPI && window.electronAPI.getSettings) {
            ...
            const threshold = settings.PASSENGER_THRESHOLD || 10;
            const logMsg = isUpdate
                ? `[AYAR] Yolcu Yükü Eşiği Güncellendi: %${threshold}`
                : `[BİLGİ] Mevcut Yolcu Yükü Eşiği: %${threshold}`;
    ...
    useEffect(() => {
        refreshSettingsLog();
        window.addEventListener('settings-updated', () => refreshSettingsLog(true));
        return () => window.removeEventListener('settings-updated', () => refreshSettingsLog(true));
    }, []);
    ...
    const handleStart = async () => {
        if (!selectedFile) {
            setLogs(prev => [...prev, "[WARN] Lütfen Hıkvısıon dosyasını seçin."]);
            return;
        }
        ...
        if (window.electronAPI && window.electronAPI.getSettings) {
            const settings = await window.electronAPI.getSettings();
            threshold = settings.PASSENGER_THRESHOLD || 10;
        }
