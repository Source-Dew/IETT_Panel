...
const HKOffline = () => {
    ...
    const [logs, setLogs] = useState(["HK Offline Modu Hazır."]);
    const [selectedFile1, setSelectedFile1] = useState(null);
    const [selectedFile2, setSelectedFile2] = useState(null);
    const [isSelectingFile, setIsSelectingFile] = useState(false);
    const [reportPath, setReportPath] = useState(null);

    // Mod konfigürasyonu (Sabit)
    const activeMode = 'offline';
    const config = {
        title: "HK Offline Saat Veri",
        desc: "Çevrimdışı araçları filtreler.",
        icon: WifiOff,
    ...
    const checkExistingReport = async () => {
        if (window.electronAPI && window.electronAPI.checkReportExists) {
            const filename = "Hıkvısıon Offline Veri (2.Yöntem).xlsx";
    ...
    const refreshSettingsLog = async (isUpdate = false) => {
        if (window.electronAPI && window.electronAPI.getSettings) {
            try {
                ...
                const threshold = settings.HK_OFFLINE_THRESHOLD || 360;
                const logMsg = `${isUpdate ? '[GÜNCELLEME]' : '[BİLGİ]'} Mevcut çevrimdışı eşiği: ${threshold} dakika`;
                setLogs(prev => {
    ...
    useEffect(() => {
        const handleFilesDrop = (e) => {
            ...
            // In the new simplified version, we just take the first two files
    ...
    const handleStart = async () => {
        ...
        let threshold = 360; // Default
        try {
            if (window.electronAPI && window.electronAPI.getSettings) {
                const settings = await window.electronAPI.getSettings();
                if (settings && settings.HK_OFFLINE_THRESHOLD) {
                    threshold = settings.HK_OFFLINE_THRESHOLD;
                    setLogs(prev => [...prev, `[BİLGİ] Çevrimdışı eşiği: ${threshold} dakika`]);
                }
            }
        } catch (err) {
            console.error("Failed to fetch settings for threshold:", err);
            setLogs(prev => [...prev, `[ERROR] Ayarlar yüklenemedi, varsayılan eşik (${threshold} dakika) kullanılacak.`]);
        }

        setStatus('running');
        setLogs(prev => [...prev, `İşlem başlatılıyor...`]);
            ...
            <div className={`flex justify-between items-center p-6 rounded-2xl border transition-colors duration-300 ${config.bg} ${config.border}`}>
                ...
                <div className="flex items-center gap-3 no-drag">
                    ...
                    >
                        ...
                        <span>{selectedFile2 ? 'Hikvision Seçildi' : 'Hıkvısıon Veri'}</span>
                    </button>
