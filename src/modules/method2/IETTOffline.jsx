...
const IETTOffline = () => {
    ...
    const [logs, setLogs] = useState(["İETT Offline Modu Hazır."]);
    const [selectedFile1, setSelectedFile1] = useState(null);
    const [selectedFile2, setSelectedFile2] = useState(null);
    const [isSelectingFile, setIsSelectingFile] = useState(false);
    const [reportPath, setReportPath] = useState(null);

    const activeMode = 'iett_offline';
    const config = {
        title: 'İETT Çevrimdışı Veri',
        desc: "İETT araçlarının çevrimdışı durumlarını raporlar.",
        icon: WifiOff,
    ...
    const checkExistingReport = async () => {
        if (window.electronAPI && window.electronAPI.checkReportExists) {
            const filename = "İETT Çevrimdışı Veri (2.Yöntem).xlsx";
            const path = await window.electronAPI.checkReportExists(filename);
            setReportPath(path); // null if not exists
        }
    };
    ...
    const refreshSettingsLog = async (isUpdate = false) => {
        if (window.electronAPI && window.electronAPI.getSettings) {
            try {
                ...
                const threshold = settings.IETT_OFFLINE_THRESHOLD || 10;
                const logMsg = `${isUpdate ? '[GÜNCELLEME]' : '[BİLGİ]'} Mevcut çevrimdışı eşiği: ${threshold} saat`;
                setLogs(prev => {
    ...
    const handleStart = async () => {
        ...
        try {
            let threshold = 10; // Default hours
            if (window.electronAPI && window.electronAPI.getSettings) {
                const settings = await window.electronAPI.getSettings();
                if (settings && settings.IETT_OFFLINE_THRESHOLD) {
                    threshold = settings.IETT_OFFLINE_THRESHOLD;
                    setLogs(prev => [...prev, `[BİLGİ] Çevrimdışı eşiği: ${threshold} saat`]);
                }
            }
            ...
            <div className={`flex justify-between items-center p-6 rounded-2xl border transition-colors duration-300 ${config.bg} ${config.border}`}>
                ...
                <div className="flex items-center gap-3 no-drag">
                    ...
                    >
                        ...
                        <span>{selectedFile2 ? 'İETT Seçildi' : 'İETT Veri'}</span>
                    </button>
                    {status === 'running' ? (
