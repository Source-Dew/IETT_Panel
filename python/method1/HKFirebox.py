...
async def process_hk_firebox(file_path, log_func):
    def log(msg, p=None): log_func(msg, p)

    try:
        log(f"HIKVISION Firebox verileri okunuyor...", 10)
        ...
        output_path = generate_premium_excel(df_report, "HIKVISION FIREBOX HATA ANALİZİ", "HIKVISION Firebox Veri (1.Yöntem).xlsx", log)
