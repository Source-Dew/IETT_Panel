...
def process_oho_trigger(files, cars2_list, cars3_list, log, detailed_cars_list="", is_detailed=False):
    ...
    for _, row in df_combined.iterrows():
        ...
        if '_' in val:
            ...
            car_code = normalize_car_code(raw_code, smart=True)
            
            # Validate if it looks like a car code (Letter + Digits)
            if not re.search(r'([A-Z])[\s_-]*(\d+)', car_code):
                log(f"⚠️ OKUNAMADI: '{str(row[col_device]).strip()}' verisi araç koduna dönüştürülemedi.")
