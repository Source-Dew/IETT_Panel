...
def process_oho_offline(file1, file2, threshold, log_func):
    def log(msg, p=None): log_func(msg, p)

    try:
        log("Atayol ve ÖHO Offline verileri karşılaştırılıyor...", 10)
        df_atayol = read_excel_smart(file1, log)
        df_oho = read_excel_smart(file2, log)

        if df_atayol.empty or df_oho.empty:
            log("Dosyalar okunamadı veya boş.", 0)
            return

        # Kolon Tespiti
        atayol_col = find_best_column(df_atayol.columns, ['PLAKA', 'KAPINO', 'KOD', 'ARAC'])
        oho_col = find_best_column(df_oho.columns, ['CIHAZ ISMI', 'PLAKA', 'KAPINO', 'ARAC'])
        time_col = find_best_column(df_oho.columns, ['ÇEVRIMDIŞI ZAMAN', 'OFFLINE TIME', 'ZAMAN', 'SAAT'])

        if not atayol_col or not oho_col or not time_col:
        ...
        # Filtreleme: ÖHO listesinde olup Atayol listesinde de olanlar
        df_oho['Normalized'] = df_oho[oho_col].apply(normalize_car_code)
        
        # Sadece G olmayan ve Atayol listesinde olan araçlar
        df_matched = df_oho[
            (df_oho['Normalized'].isin(atayol_set)) & 
            (~df_oho['Normalized'].str.startswith('G', na=False))
        ].copy()
        ...
        df_report['Plaka/Atayol No'] = df_final[oho_col]
        df_report['Çevrimdışı Süre (Saat)'] = df_final[time_col]
        df_report['Sefer Durumu'] = 'Seferi Var (Aktif)'
        
        df_report = df_report.sort_values(by=['numeric_time'], ascending=False)

        output_path = generate_premium_excel(df_report, "ÖHO ÇEVRİMDIŞI ANALİZ (2.YÖNTEM)", "OHO Çevrimdışı Analiz (2.Yöntem).xlsx", log)
